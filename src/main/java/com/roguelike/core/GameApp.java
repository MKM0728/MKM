...
public class GameApp extends GameApplication {
    ...
    protected void initGame() {
        ...
        for (int y = 0; y < ROWS; y++)
            for (int x = 0; x < COLS; x++) {
                ...
                r.setX(x * TILESIZE); r.setY(y * TILESIZE);
                r.setVisible(false);
                tileRects[y][x] = r;
    ...
    private void newGame() {
        ...
        floor = 1; turns = 0; enemiesSlain = 0;
        startFloor();
    }
    ...
    private void startFloor() {
        ...
        for (int i = 0; i < enemies.size(); i++) {
            ...
            r.setVisible(false);
            enemyRects.add(r); worldGroup.getChildren().add(r);
        }

        visible = fov.compute(dungeon, playerX(), playerY());
        markExplored();
        worldGroup.setVisible(true); playerRect.toFront();
        state = GameState.PLAYING; playerActed = false;
        renderAll(); updateHud();
    }
    ...
    private void renderAll() {
        ...
        for (int i = 0; i < enemyRects.size(); i++) {
            if (i >= enemies.size()) { enemyRects.get(i).setVisible(false); continue; }
            var e = enemies.get(i); var ep = e.get(PositionComponent.class);
            if (ep == null || !visible[ep.y()][ep.x()]) { enemyRects.get(i).setVisible(false); continue; }
            int sx = ep.x() - ox, sy = ep.y() - oy;
            if (sx < 0 || sx >= COLS || sy < 0 || sy >= ROWS) { enemyRects.get(i).setVisible(false); continue; }
            enemyRects.get(i).setX(sx * TILESIZE + 1); enemyRects.get(i).setY(sy * TILESIZE + 1);
    ...
    private void spawnEnemies() {
        ...
        for (int i = 0; i < count; i++) {
            int x, y;
            do { x = 2 + (int)(Math.random() * (GameConfig.MAP_WIDTH - 4));
    ...
    private void spawnItems() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            items.add(switch (rng.nextInt(3)) {
    ...
    private void tryMove(int dx, int dy) {
        ...
        for (var e : enemies) {
            ...
            if (ep != null && ep.x() == nx && ep.y() == ny) {
                AttackSystem.meleeAttack(player, e);
                if (!e.get(HealthComponent.class).isAlive()) enemiesSlain++;
                playerActed = true; turns++;
    ...
    private void pickupItem(Entity item) { items.remove(item); }
    private List<Entity> inventoryItems() { return new ArrayList<>(); }
    private void useItem(Entity item) {
        var ic = item.get(ItemComponent.class); if (ic == null) return;
        var hp = player.get(HealthComponent.class);
        if (hp != null && ic.itemType() == ItemType.POTION) hp.heal(ic.value());
        playerActed = true; turns++;
    }
    private void dropItem(Entity item) {
        var pp = player.get(PositionComponent.class);
        item.get(PositionComponent.class).set(pp.x(), pp.y());
        items.add(item); playerActed = true; turns++;
    }
    private void toggleInventory() { if (inventory.isVisible()) inventory.hide(); else inventory.show(inventoryItems()); }
    private void togglePause() { state = state == GameState.PLAYING ? GameState.PAUSED : GameState.PLAYING; }
    ...
    private void enemyTurns() {
        ...
        for (var e : new ArrayList<>(enemies)) {
            ...
            var ep = e.get(PositionComponent.class); var hp = e.get(HealthComponent.class);
            if (ep == null || hp == null) continue;
            boolean sees = fov.isVisible(playerX(), playerY()) &&
                Math.abs(ep.x() - playerX()) <= 8 && Math.abs(ep.y() - playerY()) <= 8 &&
                lineOfSight(ep.x(), ep.y(), playerX(), playerY());
            var s = bt.update(sees, hp.hpRatio(), ep.x(), ep.y(), playerX(), playerY());
            switch (s) {
                case ATTACK -> { AttackSystem.meleeAttack(e, player); if (!player.get(HealthComponent.class).isAlive()) { gameOver(); return; } }
                case CHASE -> moveEnemy(e, ep, playerX(), playerY());
                case FLEE -> { var f = bt.getFleeTarget(ep.x(), ep.y(), playerX(), playerY()); moveEnemy(e, ep, clamp(f[0],1,GameConfig.MAP_WIDTH-2), clamp(f[1],1,GameConfig.MAP_HEIGHT-2)); }
                case WANDER -> { var w = bt.getWanderTarget(ep.x(), ep.y(), 2,2,GameConfig.MAP_WIDTH-3,GameConfig.MAP_HEIGHT-3); moveEnemy(e, ep, w[0], w[1]); }
            }
        }
    }
    ...
    private void moveEnemy(Entity e, PositionComponent ep, int tx, int ty) {
        ...
        if (!p.isEmpty()) { var s = p.get(0); if (isTileFree(s.x(), s.y())) ep.set(s.x(), s.y()); }
    }
    ...
    private void saveGame(String slot) { try { SaveManager.save(player, floor, seed, slot); } catch (Exception ex) {} }
    private void loadGame(String slot) {
        try { var d = SaveManager.load(slot); if (d==null) { newGame(); return; }
            floor=d.floor; seed=d.seed; startFloor();
            player.get(PositionComponent.class).set(d.playerX,d.playerY);
            player.get(HealthComponent.class).takeDamage(player.get(HealthComponent.class).hp()-d.playerHp);
        } catch (Exception ex) { newGame(); }
    }

    private void gameOver() { state = GameState.GAME_OVER; hud.remove(); worldGroup.setVisible(false);
